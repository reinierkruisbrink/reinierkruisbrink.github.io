...
Friedman argued that a corporation's primary social responsibility is to maximize profits for its shareholders. True Cost Accounting acknowledges this drive but proposes that ethical outcomes can emerge when the profit motive is aligned with socially beneficial products and processes through a reformed pricing mechanism that internalizes externalities.
...
### 2.2 From Bartering to Money, and Abstracting to 4D Costs
...
We propose a new True Cost Accounting abstraction: a **4-Dimensional (4D) cost vector** that re-integrates these previously externalized dimensions back into the perceived "cost" of a product or service. This is not a return to bartering, but a sophisticated evolution of monetary systems to reflect a more complete picture of value and impact. The concept of "costs proportional to price" becomes "True Cost Accounting", where the final price (or an adjusted price signal) reflects not just economic value, but also social, environmental, and health impacts.

---

## 3. Method: The 4D Cost System and Its Implementation

### 3.1 The Four-Dimensional Cost Vector

Each product or service is represented by a 4D vector, **C**, encapsulating its comprehensive impact:
...
- **$C_2$: Social Externality** — Labour practices, community impact, human rights, fair wages, social equity
- **$C_3$: Environmental Externality** — Carbon footprint, water usage, pollution, biodiversity loss, resource depletion
- **$C_4$: Health Externality** — Public health impacts, consumer health effects, worker safety, nutritional quality

The **Total True Cost** of a product or service is calculated using a weighted norm of this vector:

$$\text{TrueCost}(C) = \sqrt{w_1C_1^2 + w_2C_2^2 + w_3C_3^2 + w_4C_4^2}$$
...
Here, $w_i$ represents the weight assigned to each dimension, allowing for flexibility based on societal priorities. A higher Total True Cost indicates a greater overall negative impact. In current economic models, this framework reduces to $w_1=1$ and $w_2=w_3=w_4=0$. Each weight ($w_i$) has a lower bound of 0.2 and an upper bound of 0.3 to prevent a (semi-)collapse to the current model. The weights should add up to 1.

### 3.2 Bundling as a Framework

The 4D vector acts as a "bundling" framework, allowing diverse and complex externalities to be aggregated into a single, quantifiable metric. Instead of separate regulations or labels for each issue, the model provides a holistic score. 

This framework is extensible; as new externalities become recognized, they can be incorporated into existing dimensions or potentially lead to the addition of new dimensions if warranted. Initially, we will start with these 4 dimensions.
...
Instead of just a final point-of-sale adjustment, the Externality Value (EV) would be subject to a tax at each transaction point:
...
2. **Introduce Externality Tax**: The revenue lost from reduced VAT would be offset (or surpassed) by the new Externality Tax levied on the 4D True Cost.
...
The 4D cost system is designed to be applicable to both products and services:
...
By implementing a 4D True Cost Accounting system that incorporates a VAT-like intermediate externality tax and partially replaces traditional VAT, we hypothesize that:
...
4. **The transparency and traceability inherent in the 4D system make lobbying harder**. This framework improves accountability by showing the 'why' behind prices.

---

## 5. Related Work: Commonalities and Missing Elements

The concept of True Cost Accounting builds upon and intersects with numerous existing initiatives aimed at addressing externalities:

- **Life Cycle Assessment (LCA)**: Provides methodologies for quantifying environmental impacts across a product's lifecycle. TCA integrates LCA data into a broader financial framework.

- **ESG (Environmental, Social, Governance) Metrics**: Corporate reporting frameworks that encourage companies to measure and disclose non-financial performance. TCA provides a mechanism to directly translate these metrics into market price signals.

- **Sustainable Certification Schemes** (e.g., Fair Trade, Organic, B Corp): These provide third-party verification of certain social or environmental standards. TCA can leverage these certifications as indicators for lower externality values.

- **Carbon Taxes/Cap-and-Trade Systems**: Existing mechanisms for pricing carbon emissions. TCA generalizes this concept to multiple dimensions of externalities.

- **Nutri-Score, Eco-Score (Open Food Facts)**: Consumer-facing labels that provide simplified scores for nutritional and environmental impact. TCA provides the underlying financial mechanism that gives these scores direct market impact.

### Commonalities

Many initiatives share the goal of transparency and incentivizing better practices.

### Missing Elements (Addressed by TCA)

- **Holistic Integration**: Most existing initiatives focus on one or two dimensions (e.g., just carbon, just nutrition). TCA provides a single, unified framework for all major externalities.

- **Direct Financial Signal**: While ESG and certifications provide information, they often lack a direct, universal financial mechanism (like a tax/price adjustment) that immediately impacts market competitiveness.

- **Distributed Accountability**: Many systems place the burden of change primarily on the consumer or the final brand. TCA distributes the financial signal and accountability throughout the entire supply chain, incentivizing and lowering the threshold for change at every step.

- **Revenue Neutrality/Tax Shifting**: TCA explicitly proposes a mechanism for tax reform (partially replacing VAT) to address regressivity and ensure stable government revenue, which is often a missing piece in pure "green tax" proposals that negatively impact low-incomes.

By combining these elements, **True Cost Accounting** aims to provide a comprehensive, actionable, and financially incentivized framework for transitioning towards more ethical and sustainable markets.

---

## 6. Experiment: Demo in the Food Industry

To demonstrate feasibility, a real-world implementation in the food industry is underway. This pilot will validate assumptions, refine the 4D model, and surface practical challenges.
