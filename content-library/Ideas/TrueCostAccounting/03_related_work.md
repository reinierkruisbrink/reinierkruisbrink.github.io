...
- Social cost categories (living wage gap, child labor, OHS) directly inform $C_2$
...
4. **Consumer health underweighted**: Strong on worker health (OHS), weaker on consumer/diet impacts
   - *TCA solution*: Explicit $C_4$ component using Nutri-Score/NOVA proxies
...
### How TCA Relates
- **LCA is a supplier**: Provides indicators for $C_3$ and some health exposure components
...
3. **Limited social integration**: LCA historically weak on labor/social dimensions
   - *TCA solution*: Equal treatment of social ($C_2$) and environmental ($C_3$) dimensions
...
### How TCA Uses Labels
- Nutri-Score and NOVA provide indicators for $C_4$ (consumer health)
- Eco-Score methodologies inform $C_3$ (environmental) indicator selection
...
- Primary data source for $C_4$ indicators (Nutri-Score, NOVA)
...
| Dimension | Existing Approaches | TCA Innovation |
|-----------|-------------------|----------------|
| **Object** | Often single-issue or narrative | Multi-dimensional vector $C=[C_1,C_2,C_3,C_4]$ in €/unit |
| **Aggregation** | Linear sums (offsetting allowed) | Norm over externalities $N_{ext}(C)$ (strong sustainability) |
| **Transparency** | Voluntary or uneven enforcement | Unknown penalty for missing/unverified data |
| **Policy** | Point-of-sale or extraction-point | VAT-like intermediate externality taxing |
| **Equity** | Often regressive (carbon taxes) | Partial VAT replacement with revenue recycling |
| **Supply Chain** | Downstream focus (labels, consumer) | Propagates incentives to all tiers |
| **Comparability** | Hard across sectors (ESG) | Unified € metric enables cross-sector comparison |
| **Gaming Resistance** | Vulnerable (cherry-picking, offsets) | Norm + unknown penalty + audit requirements |
...
**Previous:** [[Ideas/TrueCostAccounting/02_proposed_solution|Proposed Solution]]  
**Next:** [[Ideas/TrueCostAccounting/04_calculating_externalities|Calculating Externalities]]  
